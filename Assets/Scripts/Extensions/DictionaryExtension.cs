...
namespace Assets.Scripts.Extensions
{
    public static class DictionaryExtension
    {
        ...
        public static List<EcsEntity> CheckEmptyNeighbors<T>(this Dictionary<Vector3Int, EcsEntity> dic, in Vector3Int cur, in int sqrDis) where T : struct
        {
            var allNeighbors = new List<EcsEntity>();
            foreach (var dir in (Direction[]) Enum.GetValues(typeof(Direction)))
            {
                if (dic.TryGetValue(GetNeighbors(dir, cur, sqrDis), out EcsEntity entity))
                {
                    if (entity.Has<T>())
